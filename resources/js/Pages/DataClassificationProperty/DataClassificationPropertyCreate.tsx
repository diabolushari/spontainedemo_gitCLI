import useCustomForm from '@/hooks/useCustomForm'
import { useMemo } from 'react'
import { FormItem } from '@/FormBuilder/FormBuilder'
import FormPage from '@/FormBuilder/FormPage'
import { BreadcrumbItemLink } from '@/Components/BreadCrumbs'

export default function DataClassificationPropertyCreate() {
  const { formData, setFormValue } = useCustomForm({
    property_type: '',
    property_value: '',
    order: 0,
  })

  const breadCrumb: BreadcrumbItemLink[] = [
    {
      item: 'Properties Index',
      link: route('data-classification-property.index'),
    },
    {
      item: 'Create Property',
      link: '',
    },
  ]

  const formItems = useMemo(() => {
    return {
      property_type: {
        label: 'Property Type',
        type: 'text',
        setValue: setFormValue('property_type'),
        placeholder: 'e.g. Data Classification Level',
      } as FormItem<string, never, never, never>,
      property_value: {
        label: 'Property Value',
        type: 'text',
        setValue: setFormValue('property_value'),
        placeholder: 'e.g. High, Low, Confidential',
      } as FormItem<string, never, never, never>,
      order: {
        label: 'Order',
        type: 'number',
        setValue: setFormValue('order'),
        placeholder: 'Display order',
      } as FormItem<number, never, never, never>,
    }
  }, [])

  return (
    <FormPage
      formItems={formItems}
      formData={formData}
      title={'Create Classification Property'}
      url={route('data-classification-property.store')}
      backUrl={route('data-classification-property.index')}
      formStyles='md:w-1/2 md:grid-cols-1'
      type={'definitions'}
      subtype={'blocks'}
      breadCrumbs={breadCrumb}
    />
  )
}
