    ...
    if (index === 1) {
      linkElement = (
        ...
        >
          ...
          <p className='ml-2 mr-4 font-small-1stop text-sm leading-none'>Previous</p>
        </Link>
      )
    }
    if (index !== 1 && index !== listLength) {
      linkElement = (
        <Link
          as='a'
          key={index.toString() + link.label}
          className={`mr-4 hidden cursor-pointer border-t border-transparent px-2 pt-3 font-small-1stop text-sm leading-none hover:border-indigo-400 hover:text-indigo-700 md:flex ${
    ...
    if (index === listLength) {
      linkElement = (
        <Link
          ...
          className='flex cursor-pointer items-center pt-3 text-gray-600 hover:text-indigo-700'
        >
          <p className='mr-3 font-small-1stop text-sm leading-none'>Next</p>
          <svg
            width={14}
            height={8}
            viewBox='0 0 14 8'
  ...
  return (
    <div className='flex w-full flex-wrap items-center justify-between gap-y-4 py-2'>
      <p className='mt-auto self-center text-sm text-gray-700'>
        Showing <span className='font-small-1stop'>{pagination.from}</span> to{' '}
        <span className='font-small-1stop'>{pagination.to}</span> of {pagination.total}
      </p>
