...
interface ArrearsList extends Model {
  office_code: string
  office_name: string
  total_arrears: number
}
const AllArrearsList = ({
  subset_id,
  column1,
  column2,
  ...
  const [graphValues] = useFetchRecord<{ data: Paginator<ArrearsList> }>(
      ...
      <table className='mt-5'>
          ...
          {graphValues?.data.data.map((value) => {
            return (
              ...
              >
                ...
                <td className=''>{formatNumber(value.total_arrears)}</td>
              </tr>
            )
          })}
        </tbody>
        <div className='flex w-full items-center gap-5'>
          ...
          <div className='ml-auto flex w-full justify-end pt-3'>
            <Link
              href={`office-rankings/Total Arrears?route=${route('finance.index')}`}
